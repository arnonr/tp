...
class Mpic extends Component
{
  public function picform($model = null, $name = null, $folder = null, $model_name = null, $type = null)
  {
      ...
      if($model->$name != NULL){
            ...
            $option['initialPreviewFileType'] = ['image'];
            $option['allowedFileExtensions'] = ['jpg','png','jpeg'];
      }
      $img_old = $name.'_old';

      $option['allowedFileExtensions'] = ['jpg','jpeg','png'];
      ...
      $option['browseClass'] = 'btn btn-success';
      $option['browseIcon'] = '<i class="fa fa-file-image"></i> ';
      $option['browseLabel'] = 'เลือกไฟล์';

      $option['fileActionSettings'] = [
        'showZoom' => false
      ];

      // $html = '<div class="form-group row field-'.$folder.'-'.$name.' required">
      //       <label class="col-md-2 col-form-label" for="'.$folder.'-'.$name.'">'.$model->getAttributeLabel($name).'</label><div class="col-md-10">';

      // $html2 = '<span class="help-block"><div class="help-block"></div></span></div></div>';
      ...
      return FileInput::widget([
          ...
          'options' => ['accept' => 'image/png, image/jpeg, image/jpg'],
  ...
  public function docform($model = null, $name = null, $folder=null, $model_name = null, $type = null)
  {
      ...
      if($model->$name != NULL){
            ...
            $option['initialPreviewAsData'] = true;
            $option['showRemove'] = false;
            $option['initialPreviewShowDelete'] = false;
            // $arr = explode(".",$model->$name);
            $option['initialCaption'] = $model->$name;
      }


      $option['allowedFileExtensions'] = ['pdf'];
      ...
      $option['showCancel'] = false;
      $option['browseClass'] = 'btn btn-success';
      $option['browseIcon'] = '<i class="fa fa-file-pdf"></i> ';
      $option['browseLabel'] = 'เลือกไฟล์';
  ...
  public function picsave($model = null, $name = null, $folder = null,$code = null)
  {
      $model->$name = UploadedFile::getInstance($model, $name);
      if($model->$name != NULL){
          // $name_pic = time() . rand();
          $model->$name->saveAs("upload/".$folder."/" . $code . '.' . $model->$name->extension);
          ...
          $model->$name = $code . '.' .  $model->$name->extension;
      }else{
          $img_old = $name.'_old';
